...
export default function ReservationForm({ dict, common }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (ok) {
      ...
      router.push(`/${locale}/reservation-nusadua/thank-you`);
    }
  };
  ...
  return (
    <section className="border-t border-gray-200 py-24 px-6 max-w-2xl mx-auto bg-white">
      ...
      <p className="text-center text-gray-600 mb-4">{dict.intro}</p>
      <p className="text-center text-sm text-gray-500 mb-14">{common.noShowNotice}</p>
      <form className="space-y-4" onSubmit={handleSubmit}>
        <fieldset disabled={status === "submitting"} className="m-0 min-w-0 space-y-4 border-0 p-0">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <select value={fields.title} onChange={update("title")} aria-label={common.titlePlaceholder} className="border p-3 text-gray-700">
