...
export default function Accordion({ items, single = true }) {
  ...
  return (
    <div className="divide-y divide-raja-black/10">
      {items.map((item, index) => {
        ...
        return (
          <div key={item.q}>
            ...
            >
              <span
                className={`font-semibold transition-colors duration-500 ease-expo group-hover:text-raja-red ${
                  isOpen ? "text-raja-red" : ""
                }`}
              >
                {item.q}
              </span>
