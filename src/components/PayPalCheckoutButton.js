 ...
 *
 * `guests`/`plan` are passed straight through to /api/paypal/create-order
 * on every click — that route recomputes the real price from its own table
 * server-side, this component never sends an amount.
 */
export default function PayPalCheckoutButton({ clientId, formType, plan, guests, onSuccess, dict }) {
  ...
  useEffect(() => {
          ...
          .Buttons({
            ...
            createOrder: async () => {
              const res = await fetch("/api/paypal/create-order", {
                ...
                body: JSON.stringify({ formType, plan, guests }),
              });
    ...
    // formType never actually changes at runtime (fixed per page); plan/guests
